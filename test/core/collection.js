...
describe('Backbone.Collection', function () {
  ...
  it.skip('chain', function () {
    ...
    expect(wrapped.map('id').max().value()).to.equal(3);
    expect(wrapped.map('id').min().value()).to.equal(0);
    expect(
      wrapped
        .filter(function (o) {
          return o.id % 2 === 0;
  ...
  it('#2612 - nested `parse` works with `Collection#set`', function () {
    ...
    expect(job.items.get(1).subItems.get(1).get('subName')).to.equal('One');
    expect(job.items.get(2).subItems.get(3).get('subName')).to.equal('Three');
    ...
    expect(job.items.length).to.equal(2);
    expect(job.items.get(1).subItems.get(1).get('subName')).to.equal('NewOne');
    expect(job.items.get(2).subItems.get(3).get('subName')).to.equal('NewThree');
  });
  ...
  it("#3711 - remove's `update` event returns multiple removed models", function () {
    ...
    collection.on('update', function (context, options) {
      ...
      expect(_.indexOf(changed.removed, model) > -1 && _.indexOf(changed.removed, model2) > -1).to
        .be['true'];
    });
