...
describe('minLength validator', () => {
  ...
  it('has default error message for string', async () => {
    ...
    expectInvalidSet(model, { name: '' }, { name: 'Name must be at least 2 characters' });

    const [, error] = await event;
    assert.equals(error, { name: 'Name must be at least 2 characters' });
  });

  it('string with length shorter than minLenght is invalid', () => {
    expectInvalidSet(model, { name: 'a' });
  });

  it('string with length equal to minLength is valid', () => {
    expectValidSet(model, { name: 'aa' });
  });

  it('string with length greater than minLength is valid', () => {
