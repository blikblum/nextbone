...
describe('Backbone.Collection', function() {
  ...
  it('update index when id changes', function() {
    ...
    expect(collection.get(0)).to.be.null;
  ...
  it('remove same model in multiple collection', function() {
    ...
    expect(col1).to.equal(m.collection);
    col1.remove(m);
    expect(null).to.equal(m.collection);
    expect(col1.length === 0).to.be.ok;
    expect(counter).to.equal(2);
  });
  ...
  it('fetch', function() {
    ...
    collection.url = '/test';
    collection.fetch();
    expect(this.syncArgs.method).to.equal('read');
    expect(this.syncArgs.model).to.equal(collection);
    expect(this.syncArgs.options.parse).to.equal(true);

    collection.fetch({ parse: false });
    expect(this.syncArgs.options.parse).to.equal(false);
  });
  ...
  it('create', function() {
    ...
    expect(this.syncArgs.method).to.equal('create');
    expect(this.syncArgs.model).to.equal(model);
  ...
  it('Underscore methods with object-style and property-style iteratee', function() {
    ...
    expect(coll.filter({ a: 0 })).to.equal(false);
